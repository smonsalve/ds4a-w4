# Despues del ejercicio 5. Multi-output callback
...
def filter_df(df, exchange_val,margin_val,start_date_val,end_date_val):
    df['YearMonth'] = df['Entry time'].dt.strftime("%y - %m")
    dff = df[(df['Exchange']==exchange_val) & (df['Margin']==int(margin_val)) & (df['Entry time']>=start_date_val) & (df['Entry time']<=end_date_val)]
    return dff
...
def update_pnl_types(exchange, leverage, start_date, end_date):
    ...
    return {'data' :[go.Bar(
                     ...
                     go.Bar(
                        ...
                        name= 'Long',
                        marker= go.bar.Marker(
                            color='rgb(245, 132, 118)'
                        )
                    )
                    ],
            'layout':  go.Layout(title=f'PnL vs Trade type',
                      height=400,
                      legend={'x': 0, 'y': 1}
                      ),
...
def update_daily_btc(exchange, leverage, start_date, end_date):
    ...
    return {'data' :[go.Scatter(
            ...
            'layout':  go.Layout(title=f'Daily BTC Price',
                      height=400,
                      #legend={'x': 0, 'y': 1}
                      ),
...
def update_balance(exchange, leverage, start_date, end_date):
    dff=filter_df(df,exchange,leverage,start_date,end_date)
    return {'data' :[go.Scatter(
                        y=dff['Exit balance']+dff['Pnl (incl fees)'],
                        x=dff['Entry time'],
                        name= 'Short',
                        mode='lines'
                        #marker= go.bar.Marker(
